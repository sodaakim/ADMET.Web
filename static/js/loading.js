...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.button.analyze').addEventListener('click', function() {
        ...
        document.getElementById('loadingOverlay').style.display = 'flex';
        // 서버 요청을 시작하는 Promise
        let fetchPromise = fetch('/screening', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
        ...
        Promise.all([fetchPromise, timeoutPromise]).then(async ([response]) => {
            ...
            if (data.image_url) {
                // 이미지 URL과 SMILES 문자열을 /result 페이지로 리디렉션
                window.location.href = `/result?image_url=${encodeURIComponent(data.image_url)}&smiles=${encodeURIComponent(smiles)}`;
            } else {
                alert('There was an error processing the SMILES string.');
            }
        }).catch(error => {
